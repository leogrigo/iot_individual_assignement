...
```math
|X[k]| \geq 0.018 \cdot \max_j |X[j]|
```

where $|X[k]|$ is the magnitude of the $k$-th FFT bin. In the current configuration, `SIGNIFICANCE_RATIO = 0.018`, so a bin is considered significant only if its magnitude is at least `1.8%` of the maximum spectral magnitude.
